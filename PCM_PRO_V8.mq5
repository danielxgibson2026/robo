...
// Horario limite para abrir novas operacoes (0 = sem limite).
// Apos esse horario o robo nao inicia novas entradas, mas gerencia posicoes abertas.
...
// Filtro usado somente para criar o C1 quando o CA for menor que LimiteCanalGrandePontos.
input double PercentualRompimentoCriarC1 = 5.0;
// Filtro usado para entrada real no rompimento do canal operacional completo.
input double PercentualRompimentoEntrada = 10.0;
// Mantido apenas para compatibilidade visual/parametros antigos. A nova logica usa os 2 filtros acima.
input double PercentualMinimoRompimentoValido = 5;

input group "---4. ENTRADAS E SAIDAS---"
// Regra fixa da estrategia:
// CA menor que LimiteCanalGrandePontos cria C1 apos filtro proprio.
// CA + C1 vira um unico canal operacional e a entrada ocorre no rompimento de qualquer extremidade.
// CA maior ou igual ao limite opera direto pelo CA, sem criar C1.
input double PercentualStopForaCanal = 10.0;
input double BufferStopPontos = 0.0;
// Quantidade maxima de operacoes por dia (0 = sem limite).
input int MaximoOperacoesPorDia = 30;
input bool ConsiderarVelaFechamentoNaRecriacao = true;
// Quantidade de niveis de take a desenhar/usar (1, 2 ou 3).
input ENUM_QUANTIDADE_TAKES QuantidadeTakes = TAKES_2;
// --- Percentuais dos niveis de take ---
// Take N1 e calculado sobre o canal operacional a partir da linha rompida.
// Base de referencia: 100 = 100% do canal operacional.
// Exemplo: TakeN1Pct=90 em canal de 1000 pts => N1 a 900 pts da linha rompida.
// N2 = 2x N1 | N3 = 3x N1.
input double TakeN1Pct = 90.0;
// Na nova logica, N2 e N3 sao calculados automaticamente:
// N2 = 2x TakeN1 | N3 = 3x TakeN1.
input double TakeN2Pct = 180.0; // mantido por compatibilidade, nao usado no calculo
input double TakeN3Pct = 270.0; // mantido por compatibilidade, nao usado no calculo
...
// Percentual do range do take (distancia entrada -> take N1) para ativar o break even.
// Ex: 50 = quando o preco atingir 50% do caminho ate o take N1.
input double PercentualRangeTakeParaBreakEven = 70.0;
// Percentual do range do take a garantir apos o break even.
// Ex: 10 = stop move para entrada + 10% do range do take.
...
// Range completo de expansao usado para calcular takes e break even da operacao atual.
double RangeTakeAtual = 0.0;
// Preco de entrada (abertura) da operacao atual, usado no break even.
double PrecoEntradaAtual = 0.0;

int DirecaoRompimento = 0;
int OperacoesHoje = 0;        // Contador de operacoes realizadas no dia
bool CanalAtualEhGrande = false;

bool TinhaPosicaoAberta = false;
datetime MomentoFechamentoUltimaOperacao = 0;

// Controle do pullback apos vela esticada
bool AguardandoPullback = false;
int DirecaoPullback = 0;
double LinhaPullback = 0.0;
double TakeFicticio = 0.0;
double StopFicticio = 0.0; // mantido apenas para compatibilidade visual antiga; nao usado na nova logica
...
// Canal operacional completo usado para entradas e takes.
...
// Gerenciamento diario por porcentagem da banca
double SaldoReferenciaDia = 0.0;
double LimitePerdaDiariaValor = 0.0;
double MetaLucroDiariaValor = 0.0;
double ResultadoFinanceiroDiaTotal = 0.0; // realizado + flutuante do robo
...
// Numero maximo de linhas de texto que o painel pode exibir.
...
// Mini Equador / suportes e resistencias por topos e fundos
...
//+------------------------------------------------------------------+
//| Retorna true se o horario atual ainda permite abrir novas        |
//| operacoes. Quando HoraLimiteOperacao == 0 e MinutoLimiteOperacao  |
//| == 0 nao ha limite (retorna sempre true).                        |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Retorna true se o limite diario de operacoes ainda nao foi       |
//| atingido. MaximoOperacoesPorDia == 0 significa sem limite.       |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Atualiza o texto e a cor do botao do painel                       |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Cria botao para ocultar/exibir o painel                           |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Retorna true quando o stop ou o take diario ja foi atingido      |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Inicializa os limites diarios a partir do saldo da sessao        |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Lucro/prejuizo flutuante apenas da posicao deste robo            |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Dispara alerta visual e sonoro conforme configuracao             |
//+------------------------------------------------------------------+
...
bool CalcularCanalPrimeirasVelas(datetime inicio, double &topo, double &fundo)
  {
   ...
   if(total <= QuantidadeVelasCanal + 2)
      return false;
...
//|                                                                  |
//+------------------------------------------------------------------+
bool CalcularCanalCorpoAposHorario(datetime inicio, bool incluirVelaInicio, double &topoCorpo, double &fundoCorpo)
  {
   int total = iBars(_Symbol, TF());
   ...
   for(int shift = total - 1; shift >= 1; shift--)
     {
      ...
      if(maxCorpo > topoCorpo)
         topoCorpo = maxCorpo;

      if(minCorpo < fundoCorpo)
         fundoCorpo = minCorpo;
   ...
   topoCorpo = NormalizarPreco(topoCorpo);
   fundoCorpo = NormalizarPreco(fundoCorpo);
...
bool VelaGrandeParaCanal(int shift)
  {
   if(!UsarFiltroVelaGrandeCanal)
      return false;

   if(TamanhoMaximoVelaCanalPontos <= 0.0)
      return false;

   double tamanhoPontos = TamanhoVelaFiltroCanalPontos(shift);

   return tamanhoPontos > TamanhoMaximoVelaCanalPontos;
  }

//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
bool CalcularCanalExpansivoAposHorario(datetime inicio, bool incluirVelaInicio, double &topo, double &fundo, int &velasUsadas)
  {
   int total = iBars(_Symbol, TF());
   ...
   // 1) Regra principal quando configurado sem pavio:
   //    tenta formar o CA pelas primeiras N velas usando somente corpo real.
   if(UsarCorpoRealSemPavio)
     {
      int contCorpo = 0;
      ...
      for(int shift = total - 1; shift >= 1; shift--)
        {
         ...
         if(contCorpo >= QuantidadeVelasCanal)
            break;
        }
      ...
      // Se as primeiras velas por corpo atingirem o minimo, o CA permanece sem pavios.
      ...
      // Se nao pode expandir ate o minimo, a excecao de pavios nao deve ser usada.
      if(!ExpandirCanalAteMinimoPontos)
         return false;

      // 2) Excecao: como o corpo nao atingiu o minimo, tenta usar pavios.
      //    Primeiro considera as mesmas primeiras N velas com pavios; se ainda nao atingir,
      //    continua contando as proximas velas com pavios ate o limite configurado.
      int contPavio = 0;
      ...
      for(int shift = total - 1; shift >= 1; shift--)
        {
         ...
         if(contPavio >= QuantidadeVelasCanal)
           {
            double tamanhoPavioPontos = (topoPavio - fundoPavio) / _Point;
            if(tamanhoPavioPontos >= CanalMinimoPontos)
               break;
           }

         if(contPavio >= limiteVelas)
            break;
        }
      ...
      if(topoPavio <= fundoPavio)
         return false;

      double tamanhoFinalPontos = (topoPavio - fundoPavio) / _Point;
      if(tamanhoFinalPontos < CanalMinimoPontos)
         return false;
   ...
   // Regra normal quando o parametro permite pavios: CA pelos pavios e expansao ate o minimo.
   int cont = 0;
   topo = -DBL_MAX;
   fundo = DBL_MAX;
   velasUsadas = 0;
      ...
      if(cont >= QuantidadeVelasCanal)
        {
         ...
         if(tamanhoPontos >= CanalMinimoPontos)
            break;
        }
   ...
   double tamanhoFinal = (topo - fundo) / _Point;

   if(tamanhoFinal < CanalMinimoPontos)
      return false;
...
//+------------------------------------------------------------------+
//| Atualiza o canal operacional completo usado para entrada/take     |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Retorna o percentual de filtro usado para rompimento              |
//+------------------------------------------------------------------+
double FiltroRompimentoPontos(double rangeReferencia, double percentual)
  {
   if(rangeReferencia <= 0.0 || percentual <= 0.0)
      return 0.0;
...
//+------------------------------------------------------------------+
//| Valida rompimento de compra com percentual especifico             |
//+------------------------------------------------------------------+
bool RompimentoValidoCompra(double fechamento, double linhaRompida, double rangeReferencia, double percentual)
  {
   if(rangeReferencia <= 0.0)
      return false;

   double distanciaMinima = FiltroRompimentoPontos(rangeReferencia, percentual);
   return fechamento >= (linhaRompida + distanciaMinima);
  }

//+------------------------------------------------------------------+
//| Valida rompimento de venda com percentual especifico              |
//+------------------------------------------------------------------+
...
bool CalcularEquadorAnual()
  {
   ...
   if(AnoEquadorAtual == anoAtual && EquadorRange > 0.0)
      return true;
   ...
   if(totalW1 <= QuantidadeSemanasEquador + 5)
      return false;
   ...
   for(int shift = totalW1 - 1; shift >= 0; shift--)
     {
      ...
      MqlDateTime dtSemana;
      TimeToStruct(tempoSemana, dtSemana);

      if(dtSemana.year != anoAtual)
         continue;
      ...
      if(EquadorUsarCorpoSemPavio)
        {
         topoVela = MathMax(o, c);
         fundoVela = MathMin(o, c);
        }
      else
        {
         topoVela = h;
      ...
      if(semanasEncontradas >= QuantidadeSemanasEquador)
         break;
     }
...
void AtualizarDistanciasEquador()
  {
   ...
   double menorN1 = DBL_MAX;
   double menorN2 = DBL_MAX;
   double menorN3 = DBL_MAX;

   int inicioK, fimK;
   EquadorRangeK(inicioK, fimK);
      ...
      if(k % 4 == 0)
        {
         if(dist < menorN1)
           {
            menorN1 = dist;
            PrecoEquadorN1MaisProximo = precoLinha;
           }
        }
      else
         if(k % 2 == 0)
           {
            if(dist < menorN2)
              {
               menorN2 = dist;
               PrecoEquadorN2MaisProximo = precoLinha;
              }
           }
         else
           {
            if(dist < menorN3)
              {
               menorN3 = dist;
               PrecoEquadorN3MaisProximo = precoLinha;
              }
           }
     }

   if(menorN1 < DBL_MAX)
      DistanciaEquadorN1Pct = menorN1 / EquadorRange * 100.0;

   if(menorN2 < DBL_MAX)
      DistanciaEquadorN2Pct = menorN2 / EquadorRange * 100.0;

   if(menorN3 < DBL_MAX)
      DistanciaEquadorN3Pct = menorN3 / EquadorRange * 100.0;
  }
   ...
   for(int k = inicioK; k <= fimK; k++)
     {
      ...
      if(ehN1 && !MostrarEquadorNivel1)
         continue;

      if(ehN2 && !MostrarEquadorNivel2)
         continue;

      if(ehN3 && !MostrarEquadorNivel3)
         continue;
      ...
      if(ehN1)
        {
         nivel = "N1_";
         cor = CorEquadorNivel1;
         estilo = EstiloEquadorNivel1;
         largura = LarguraEquadorNivel1;
        }
      else
         if(ehN2)
           {
            nivel = "N2_";
            cor = CorEquadorNivel2;
            estilo = EstiloEquadorNivel2;
            largura = LarguraEquadorNivel2;
           }
   ...
   for(int i = total - 1; i >= 0; i--)
     {
      ...
      if(StringFind(nome, prefixoTF) == 0)
         ObjectDelete(0, nome);
     }
  }
...
bool PrecoJaMarcado(double preco, double &lista[], int total, double toleranciaPontos)
  {
   for(int i = 0; i < total; i++)
     {
      if(MathAbs(preco - lista[i]) <= toleranciaPontos * _Point)
         return true;
     }
...
void DesenharTopoFundoTimeframe(ENUM_TIMEFRAMES tf, string tagTF, int barrasAnalise, color cor)
  {
   ...
   if(totalBarras <= ForcaSwingTopoFundo * 2 + 10)
      return;
   ...
   for(int shift = ForcaSwingTopoFundo + 1; shift <= maxShift; shift++)
     {
      if(linhasCriadas >= MaxLinhasTopoFundoPorTimeframe)
         break;

      if(EhTopoSwing(tf, shift, ForcaSwingTopoFundo))
        {
         double precoTopo = NormalizarPreco(iHigh(_Symbol, tf, shift));

         if(!PrecoJaMarcado(precoTopo, precosMarcados, totalMarcados, 20.0))
           {
            precosMarcados[totalMarcados++] = precoTopo;
            string nome = PREFIXO_MINI_EQUADOR + tagTF + "_TOPO_" + IntegerToString(linhasCriadas);
            CriarLinhaTopoFundo(nome, precoTopo, cor, EstiloTopoFundo, LarguraTopoFundo, tagTF + " TOPO");
            linhasCriadas++;
           }
        }
      ...
      if(EhFundoSwing(tf, shift, ForcaSwingTopoFundo))
        {
         double precoFundo = NormalizarPreco(iLow(_Symbol, tf, shift));

         if(!PrecoJaMarcado(precoFundo, precosMarcados, totalMarcados, 20.0))
           {
            precosMarcados[totalMarcados++] = precoFundo;
            string nome = PREFIXO_MINI_EQUADOR + tagTF + "_FUNDO_" + IntegerToString(linhasCriadas);
            CriarLinhaTopoFundo(nome, precoFundo, cor, EstiloTopoFundo, LarguraTopoFundo, tagTF + " FUNDO");
            linhasCriadas++;
           }
        }
     }
...
void DesenharMiniEquadorTopoFundo()
  {
   if(!MostrarTopoFundoImportante)
     {
      LimparLinhasMiniEquador();
      return;
     }

   if(MostrarTopoFundoD1)
      DesenharTopoFundoTimeframe(PERIOD_D1, "D1", BarrasAnaliseTopoFundoD1, CorTopoFundoD1);
   else
      LimparLinhasMiniEquadorTF("D1");

   if(MostrarTopoFundoH4)
      DesenharTopoFundoTimeframe(PERIOD_H4, "H4", BarrasAnaliseTopoFundoH4, CorTopoFundoH4);
   else
      LimparLinhasMiniEquadorTF("H4");

   if(MostrarTopoFundoH1)
      DesenharTopoFundoTimeframe(PERIOD_H1, "H1", BarrasAnaliseTopoFundoH1, CorTopoFundoH1);
   else
      LimparLinhasMiniEquadorTF("H1");
...
void AtualizarMiniEquadorSeNecessario()
  {
   ...
   if(tD1 != UltimaAtualizacaoD1)
     {
      UltimaAtualizacaoD1 = tD1;
      atualizar = true;
     }

   if(tH4 != UltimaAtualizacaoH4)
     {
      UltimaAtualizacaoH4 = tH4;
      atualizar = true;
     }

   if(tH1 != UltimaAtualizacaoH1)
     {
      UltimaAtualizacaoH1 = tH1;
      atualizar = true;
     }
...
bool FormarCanalAutomatico()
  {
   ...
   if(MedirLimiteCanalGrandeSomenteCorpo)
     {
      double corpoCalculado = CalcularTamanhoCorpoPrimeirasVelasApos(SessaoAtual);
      if(corpoCalculado > 0.0)
         CA_TamanhoCorpo = corpoCalculado;
     }
...
bool FormarNovoCanalAposFechamento()
  {
   if(MomentoFechamentoUltimaOperacao <= 0)
      return false;
   ...
   // Excecao: reset por take virtual na propria vela de rompimento.
   // Neste caso, a vela grande deve ser ignorada e a contagem inicia somente na proxima vela.
   if(CandleInicioNovaContagem > 0)
     {
      baseContagem = CandleInicioNovaContagem;
      incluirInicio = false;
     }
   ...
   if(MedirLimiteCanalGrandeSomenteCorpo)
     {
      double corpoCalculado = CalcularTamanhoCorpoPrimeirasVelasApos(baseContagem);
      if(corpoCalculado > 0.0)
         CA_TamanhoCorpo = corpoCalculado;
     }
...
bool FormarCanalManual()
  {
   ...
   if(!CalcularCanalUltimasVelas(topo, fundo))
      return false;
      ...
      for(int shift = QuantidadeVelasCanal; shift >= 1; shift--)
        {
         ...
         if(MathMax(o, c) > topoCorpo)
            topoCorpo  = MathMax(o, c);
         if(MathMin(o, c) < fundoCorpo)
            fundoCorpo = MathMin(o, c);
        }
...
//+------------------------------------------------------------------+
//| RECALCULO MANUAL DAS LINHAS                                      |
//+------------------------------------------------------------------+
void RecalcularC1PorCanalAtual()
  {
   if(Estado != ESTADO_AGUARDANDO_ROMPIMENTO_C1)
      return;

   if(DirecaoRompimento == 1)
     {
      C1_Fundo = CA_Topo;
      C1_Topo = CA_Topo + CA_Tamanho;
     }
   else
      if(DirecaoRompimento == -1)
        {
         C1_Topo = CA_Fundo;
         C1_Fundo = CA_Fundo - CA_Tamanho;
        }
      else
        {
         return;
...
bool AtualizarCanalPorLinhasManuais()
  {
   if(BloquearAtualizacaoManual)
      return false;

   if(!PermitirAjusteManualLinhas)
      return false;
   ...
   if(topoLinha <= 0.0 || fundoLinha <= 0.0)
      return false;

   if(topoLinha < fundoLinha)
     {
      double temp = topoLinha;
      topoLinha = fundoLinha;
      fundoLinha = temp;
     }
   ...
   bool mudou = (MathAbs(novoTopo - CA_Topo) > (_Point * 0.5) || MathAbs(novoFundo - CA_Fundo) > (_Point * 0.5));
...
//+------------------------------------------------------------------+
//| CALCULO DOS NIVEIS DE TAKE                                       |
//+------------------------------------------------------------------+
// direcao      : 1=compra, -1=venda
// baseTake     : preco de partida da expansao
//                  entrada direta CA: topo/fundo do CA
//                  entrada pelo C1:   topo/fundo do C1
// rangeExpansao: tamanho total sobre o qual aplicar os percentuais
//                  sem C1: CA_Tamanho
//                  com C1: CA_Tamanho + C1_Tamanho
// TakeN1Pct define o primeiro alvo. N2 e N3 sao gerados automaticamente como 2x e 3x o N1.
// Base de referencia: 100 = 100% do canal operacional.
void CalcularNiveisTake(int direcao, double baseTake, double rangeExpansao)
  {
   double distanciaN1 = rangeExpansao * (TakeN1Pct / 100.0);
   double distanciaN2 = distanciaN1 * 2.0;
   double distanciaN3 = distanciaN1 * 3.0;

   if(direcao == 1)
     {
      LinhaTakeN1 = NormalizarPreco(baseTake + distanciaN1);
      LinhaTakeN2 = NormalizarPreco(baseTake + distanciaN2);
      LinhaTakeN3 = NormalizarPreco(baseTake + distanciaN3);
     }
   else
     {
      LinhaTakeN1 = NormalizarPreco(baseTake - distanciaN1);
   ...
   // Guarda a distancia do take N1 para uso no break even.
   RangeTakeAtual = distanciaN1;
  }
...
bool PodeComprar()
  {
   return DirecaoPermitida == DIRECAO_AMBOS || DirecaoPermitida == DIRECAO_COMPRA;
  }
...
bool PodeVender()
  {
   return DirecaoPermitida == DIRECAO_AMBOS || DirecaoPermitida == DIRECAO_VENDA;
  }
...
void ExecutarCompra()
  {
   ...
   if(CanalOperacionalTamanho <= 0.0)
      return;
   ...
   if(linhaBaseTake <= 0.0)
      linhaBaseTake = CanalOperacionalTopo;
...
void ExecutarVenda()
  {
   ...
   if(linhaBaseTake <= 0.0)
      linhaBaseTake = CanalOperacionalFundo;
...
bool VelaRompimentoEsticada(int shift, double rangeReferencia)
  {
   ...
   if(UsarCorpoParaFiltroVela)
      tamanhoVela = MathAbs(close - open);
   else
      tamanhoVela = MathAbs(high - low);
...
double CalcularStopVirtualCompra()
  {
   ...
   if(C1_Fundo > 0.0 && C1_Fundo < extremoInferiorStop)
      extremoInferiorStop = C1_Fundo;
...
double CalcularStopVirtualVenda()
  {
   ...
   if(C1_Topo > 0.0 && C1_Topo > extremoSuperiorStop)
      extremoSuperiorStop = C1_Topo;
...
void DesenharPullbackVirtual()
  {
   if(!MostrarLinhasPullbackVirtual)
      return;
...
//+------------------------------------------------------------------+
//| Inicia espera de pullback com confirmacao obrigatoria             |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Cancela a tentativa de pullback e volta a esperar operacao        |
//+------------------------------------------------------------------+
void CancelarPullbackVoltarEsperaOperacao()
  {
   ...
   if(LimiteDiarioAtingido())
      ...
      if(PodeFazerMaisOperacoes())
        {
         AtualizarCanalOperacional();
         Estado = (C1_Topo > 0.0 && C1_Fundo > 0.0) ? ESTADO_AGUARDANDO_ROMPIMENTO_C1 : ESTADO_AGUARDANDO_ROMPIMENTO_CA;
        }
      else
         Estado = ESTADO_OPERADO_DIA;
  }
...
//+------------------------------------------------------------------+
//| Reset por take virtual sem operacao                              |
//+------------------------------------------------------------------+
void ResetarPorTakeVirtual()
  {
   ...
   // Se a propria vela do rompimento esticado ja atingiu o take virtual,
   // a nova contagem deve comecar somente a partir da proxima vela.
...
//+------------------------------------------------------------------+
//| Rompimento minimo usando filtro antigo por compatibilidade        |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Verifica pullback com confirmacao obrigatoria                     |
//+------------------------------------------------------------------+
   ...
   if(DirecaoPullback == 1)
     {
      ...
      if(atingiuTakeVirtual)
        {
         ResetarPorTakeVirtual();
         return;
        }

      if(!PullbackTocado)
        {
         bool tocouPullbackCompra = false;

         if(PullbackSomenteComFechamentoVela)
            tocouPullbackCompra = (close1 <= LinhaPullback - BufferPullbackPontos * _Point);
         else
            tocouPullbackCompra = (bid <= LinhaPullback - BufferPullbackPontos * _Point || low1 <= LinhaPullback - BufferPullbackPontos * _Point);

         if(tocouPullbackCompra)
           {
            PullbackTocado = true;
        ...
        {
         if(candleFechado <= CandlePullbackTocado)
            return;

         bool confirmouCompra = (close1 > LinhaRompidaOperacao);
         ...
         // Tocou o pullback, mas nao confirmou retomada: cancela a tentativa e volta a esperar novo rompimento.
      ...
      if(DirecaoPullback == -1)
        {
         ...
         if(!PullbackTocado)
           {
            bool tocouPullbackVenda = false;

            if(PullbackSomenteComFechamentoVela)
               tocouPullbackVenda = (close1 >= LinhaPullback + BufferPullbackPontos * _Point);
            else
               tocouPullbackVenda = (ask >= LinhaPullback + BufferPullbackPontos * _Point || high1 >= LinhaPullback + BufferPullbackPontos * _Point);

            if(tocouPullbackVenda)
              {
               PullbackTocado = true;
               CandlePullbackTocado = candleFechado;
               MaximaVelaPullback = high1;
         ...
         else
           {
            ...
            bool confirmouVenda = (close1 < LinhaRompidaOperacao);
...
//+------------------------------------------------------------------+
//| Entrada de compra com filtro de vela esticada                     |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Entrada de venda com filtro de vela esticada                      |
//+------------------------------------------------------------------+
...
void VerificarRompimentoCanalAbertura()
  {
   ...
   bool rompeuCompra = false;
   bool rompeuVenda = false;
   ...
   if(rompeuCompra)
     {
      ...
      if(!canalGrande)
        {
         // CA pequeno: este rompimento serve somente para criar C1.
         if(!RompimentoValidoCompra(close1, CA_Topo, CA_Tamanho, PercentualRompimentoCriarC1))
            return;
      ...
      // CA grande: nao cria C1; aguarda rompimento de entrada com filtro proprio.
      if(!RompimentoValidoCompra(close1, CA_Topo, CA_Tamanho, PercentualRompimentoEntrada))
         return;
   ...
   if(rompeuVenda)
     {
      ...
      if(!canalGrande)
        {
         if(!RompimentoValidoVenda(close1, CA_Fundo, CA_Tamanho, PercentualRompimentoCriarC1))
            return;
      ...
      if(!RompimentoValidoVenda(close1, CA_Fundo, CA_Tamanho, PercentualRompimentoEntrada))
         return;
...
//+------------------------------------------------------------------+
//| Rompimento do canal operacional CA+C1                             |
//+------------------------------------------------------------------+
void VerificarRompimentoC1()
  {
   ...
   // A vela que criou o C1 nao pode, no mesmo candle, disparar a entrada real.
   if(mesmoCandleCriacaoC1)
      return;

   // A partir daqui, CA + C1 vira um unico canal operacional.
   // A entrada pode ocorrer no rompimento de qualquer uma das extremidades desse canal.
   if(close1 > CanalOperacionalTopo)
     {
      if(!PodeComprar())
         return;

      if(!RompimentoValidoCompra(close1, CanalOperacionalTopo, CanalOperacionalTamanho, PercentualRompimentoEntrada))
         return;

      LinhaRompidaOperacao = CanalOperacionalTopo;
      double takeFakeCompra = LinhaRompidaOperacao + (CanalOperacionalTamanho * (TakeN1Pct / 100.0));
      ProcessarEntradaCompraComFiltro(LinhaRompidaOperacao, takeFakeCompra, CanalOperacionalTamanho);
      return;
     }
   ...
   if(close1 < CanalOperacionalFundo)
     {
      ...
      if(!RompimentoValidoVenda(close1, CanalOperacionalFundo, CanalOperacionalTamanho, PercentualRompimentoEntrada))
         return;

      LinhaRompidaOperacao = CanalOperacionalFundo;
      double takeFakeVenda = LinhaRompidaOperacao - (CanalOperacionalTamanho * (TakeN1Pct / 100.0));
      ProcessarEntradaVendaComFiltro(LinhaRompidaOperacao, takeFakeVenda, CanalOperacionalTamanho);
      return;
     }
  }
...
void ProcessarRompimentosAposAjusteManual()
  {
   if(!VerificarEntradaAposMoverLinhas)
      return;
...
//+------------------------------------------------------------------+
//| DETECTAR FECHAMENTO E PREPARAR NOVO CICLO                        |
//+------------------------------------------------------------------+
...
void GerenciarBreakEven()
  {
   ...
   // Distancia em preco para ativar o break even = % do range do take
   double distanciaAtivacao = RangeTakeAtual * PercentualRangeTakeParaBreakEven / 100.0;
   // Distancia em preco garantida apos o break even = % do range do take
   double distanciaGarantia = RangeTakeAtual * PercentualRangeTakeAposBreakEven / 100.0;

   if(tipo == POSITION_TYPE_BUY)
     {
      double lucroPreco = bid - precoEntrada;

      if(lucroPreco >= distanciaAtivacao)
        {
         double novoSL = NormalizarPreco(precoEntrada + distanciaGarantia);

         if(slAtual < novoSL)
            trade.PositionModify(_Symbol, novoSL, tpAtual);
        }
     }
   else
...
//+------------------------------------------------------------------+
//| Calcula o resultado diario total: realizado + flutuante          |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Aplica stop diario e alvo diario em % da banca                   |
//+------------------------------------------------------------------+
void GerenciarLimitesDiarios()
  {
   ...
   if(!atingiuStop && !atingiuTake)
      return;

   if(atingiuStop)
      StopDiarioAtingido = true;
   else
      TakeDiarioAtingido = true;
      ...
      if(precoEntrada > 0.0)
        {
         ...
         if(tipoEntrada == DEAL_TYPE_BUY)
            pontos = (precoSaida - precoEntrada) / _Point;
         else
            if(tipoEntrada == DEAL_TYPE_SELL)
               pontos = (precoEntrada - precoSaida) / _Point;
...
//| PAINEL                                                           |
//+------------------------------------------------------------------+
void LimparLinhasPainelAntigas(int quantidadeMaxima)
  {
...
void MostrarStatus()
  {
   ...
   if(Estado == ESTADO_AGUARDANDO_CANAL)
      estadoTxt = "Aguardando formar CA";
   else
      if(Estado == ESTADO_AGUARDANDO_ROMPIMENTO_CA)
         estadoTxt = "Aguardando rompimento CA";
      else
         if(Estado == ESTADO_AGUARDANDO_ROMPIMENTO_C1)
            estadoTxt = "Aguardando rompimento C1";
         else
            if(Estado == ESTADO_OPERADO_DIA)
               estadoTxt = "Limite de operacoes atingido";
            else
               if(Estado == ESTADO_AGUARDANDO_PULLBACK)
                  estadoTxt = "Aguardando pullback";
               else
                  if(Estado == ESTADO_LIMITE_DIARIO_ATINGIDO)
                    {
                     if(StopDiarioAtingido)
                        estadoTxt = "Stop diario atingido";
                     else
                        if(TakeDiarioAtingido)
                           estadoTxt = "Take diario atingido";
                    }
   ...
   CriarOuAtualizarTextoPainel(linha++, "FIMATHE PCM PRO", x, y, corTitulo, 12);
   y += passo + 2;
   CriarOuAtualizarTextoPainel(linha++, "Estado: " + estadoTxt, x, y, corTexto, fonte);
   y += passo;
   CriarOuAtualizarTextoPainel(linha++, "Direcao: " + direcaoTxt, x, y, corTexto, fonte);
   y += passo;

   // Operacoes do dia e horario limite
   string opsTxt = IntegerToString(OperacoesHoje);
   if(MaximoOperacoesPorDia > 0)
      opsTxt += "/" + IntegerToString(MaximoOperacoesPorDia);
   else
      opsTxt += " (sem limite)";
   CriarOuAtualizarTextoPainel(linha++, "Operacoes hoje: " + opsTxt, x, y,
                                (PodeFazerMaisOperacoes() ? corTexto : clrTomato), fonte);
   y += passo;
   CriarOuAtualizarTextoPainel(linha++, "Horario limite: " + horarioLimiteTxt, x, y,
                                (DentroDoHorarioOperacao() ? corTexto : clrTomato), fonte);
   y += passo;

   CriarOuAtualizarTextoPainel(linha++, "Saldo ref. dia: " + DoubleToString(SaldoReferenciaDia, 2), x, y, corTexto, fonte);
   y += passo;
   CriarOuAtualizarTextoPainel(linha++, "Resultado dia total: " + DoubleToString(ResultadoFinanceiroDiaTotal, 2), x, y,
                                (ResultadoFinanceiroDiaTotal >= 0.0 ? clrLime : clrTomato), fonte);
   y += passo;
   ...
   CriarOuAtualizarTextoPainel(linha++, "Stop diario: " + stopDiarioTxt, x, y,
                                (StopDiarioAtingido ? clrTomato : corTexto), fonte);
   y += passo;
   CriarOuAtualizarTextoPainel(linha++, "Take diario: " + takeDiarioTxt, x, y,
                                (TakeDiarioAtingido ? clrLime : corTexto), fonte);
   y += passo;
   CriarOuAtualizarTextoPainel(linha++, "Status diario: " + (LimiteDiarioAtingido() ? "BLOQUEADO" : "LIBERADO"), x, y,
                                (LimiteDiarioAtingido() ? clrTomato : clrLime), fonte);
   y += passo;

   CriarOuAtualizarTextoPainel(linha++, "------------------------------", x, y, clrSilver, fonte);
   y += passo;

   if(CA_Tamanho > 0.0)
     {
      CriarOuAtualizarTextoPainel(linha++, "Canal abertura: " + DoubleToString(CA_Tamanho / _Point, 1) + " pts", x, y, corTexto, fonte);
      y += passo;
      CriarOuAtualizarTextoPainel(linha++, "CA corpo limite: " + DoubleToString(CA_TamanhoCorpo / _Point, 1) + " pts", x, y, corTexto, fonte);
      y += passo;
      CriarOuAtualizarTextoPainel(linha++, "Canal grande: " + (CanalAtualEhGrande ? "SIM" : "NAO"), x, y, (CanalAtualEhGrande ? clrOrange : clrLime), fonte);
      y += passo;
     }
   else
     {
      CriarOuAtualizarTextoPainel(linha++, "Canal abertura: aguardando", x, y, corTexto, fonte);
      y += passo;
     }
   ...
   // Takes: mostra apenas os niveis ativos conforme QuantidadeTakes
   if(LinhaTakeN1 > 0.0)
     {
      CriarOuAtualizarTextoPainel(linha++, "Take N1: " + DoubleToString(LinhaTakeN1, _Digits) +
                                  " (" + DoubleToString(TakeN1Pct, 1) + ")", x, y, CorTake, fonte);
      y += passo;

      if((int)QuantidadeTakes >= 2 && LinhaTakeN2 > 0.0)
        {
         CriarOuAtualizarTextoPainel(linha++, "Take N2: " + DoubleToString(LinhaTakeN2, _Digits) +
                                     " (" + DoubleToString(TakeN1Pct * 2.0, 1) + ")", x, y, CorTake, fonte);
         y += passo;
        }
      ...
      if((int)QuantidadeTakes >= 3 && LinhaTakeN3 > 0.0)
        {
         CriarOuAtualizarTextoPainel(linha++, "Take N3: " + DoubleToString(LinhaTakeN3, _Digits) +
                                     " (" + DoubleToString(TakeN1Pct * 3.0, 1) + ")", x, y, CorTake, fonte);
         y += passo;
        }

      if(RangeTakeAtual > 0.0)
        {
         CriarOuAtualizarTextoPainel(linha++, "Range take: " + DoubleToString(RangeTakeAtual / _Point, 1) + " pts", x, y, clrSilver, fonte);
         y += passo;
        }
     }
   ...
   if(AguardandoPullback)
     {
      CriarOuAtualizarTextoPainel(linha++, "Pullback: AGUARDANDO", x, y, clrYellow, fonte);
      y += passo;
      CriarOuAtualizarTextoPainel(linha++, "Linha pullback: " + DoubleToString(LinhaPullback, _Digits), x, y, clrYellow, fonte);
      y += passo;
      CriarOuAtualizarTextoPainel(linha++, "Confirmacao: " + (PullbackTocado ? "AGUARDANDO RETOMADA" : "AGUARDANDO TOQUE"), x, y, clrYellow, fonte);
      y += passo;
     }
   ...
   if(UsarBreakEven && RangeTakeAtual > 0.0)
     {
      CriarOuAtualizarTextoPainel(linha++, "------------------------------", x, y, clrSilver, fonte);
      y += passo;
      CriarOuAtualizarTextoPainel(linha++, "BREAK EVEN", x, y, clrAqua, 11);
      y += passo;
      ...
      CriarOuAtualizarTextoPainel(linha++, "Ativa em: " + DoubleToString(ptosAtivacao, 0) + " pts (" +
                                  DoubleToString(PercentualRangeTakeParaBreakEven, 0) + "% range)", x, y, corTexto, fonte);
      y += passo;
      CriarOuAtualizarTextoPainel(linha++, "Garante: " + DoubleToString(ptosGarantia, 0) + " pts (" +
                                  DoubleToString(PercentualRangeTakeAposBreakEven, 0) + "% range)", x, y, corTexto, fonte);
      y += passo;
     }
   ...
   if(MostrarLinhasEquador && EquadorRange > 0.0)
     {
      CriarOuAtualizarTextoPainel(linha++, "EQUADOR", x, y, clrAqua, 11);
      y += passo;
      CriarOuAtualizarTextoPainel(linha++, "N1 prox: " + DoubleToString(DistanciaEquadorN1Pct, 1) + "%", x, y, CorEquadorNivel1, fonte);
      y += passo;
      CriarOuAtualizarTextoPainel(linha++, "N2 prox: " + DoubleToString(DistanciaEquadorN2Pct, 1) + "%", x, y, CorEquadorNivel2, fonte);
      y += passo;
      CriarOuAtualizarTextoPainel(linha++, "N3 prox: " + DoubleToString(DistanciaEquadorN3Pct, 1) + "%", x, y, CorEquadorNivel3, fonte);
      y += passo;
      CriarOuAtualizarTextoPainel(linha++, "------------------------------", x, y, clrSilver, fonte);
      y += passo;
     }

   CriarOuAtualizarTextoPainel(linha++, "PLACAR DO DIA", x, y, corPlacar, 11);
   y += passo;
   CriarOuAtualizarTextoPainel(linha++, "Entradas: " + IntegerToString(TotalEntradasDia), x, y, corPlacar, fonte);
   y += passo;
   CriarOuAtualizarTextoPainel(linha++, "Gains: " + IntegerToString(PlacarGainDia), x, y, clrLime, fonte);
   y += passo;
   CriarOuAtualizarTextoPainel(linha++, "Losses: " + IntegerToString(PlacarLossDia), x, y, clrTomato, fonte);
   y += passo;
   CriarOuAtualizarTextoPainel(linha++, "Resultado pontos: " + DoubleToString(ResultadoPontosDia, 1), x, y, (ResultadoPontosDia >= 0.0 ? clrLime : clrTomato), fonte);
   y += passo;
   CriarOuAtualizarTextoPainel(linha++, "Saldo do dia: " + DoubleToString(SaldoFinanceiroDia, 2), x, y, (SaldoFinanceiroDia >= 0.0 ? corSaldo : corNegativo), 11);
   y += passo;
   ...
   if(AlertaAoTakeStop)
     {
      if(reason == DEAL_REASON_TP)
         DispararAlerta("TAKE atingido");
      else
         if(reason == DEAL_REASON_SL)
            DispararAlerta("STOP atingido");
     }
...
void OnTick()
  {
   if(MostrarLinhasEquador)
     {
      MqlDateTime dtAgora;
      TimeToStruct(TimeCurrent(), dtAgora);
   ...
   if(mudouManual)
      ProcessarRompimentosAposAjusteManual();
